...
class ScenarioGeneratorConfig:
    ...
    width = 4.0
    amount_pp = 2  # initially added planning problems
    max_amount_pp = 2  # max amount of planning problems in a scenario
    min_amount_pp = 2
    do_mutations = False  # for debugging
    max_position_modifier = 50  # max amount for position mutations; min amount is -max_position_modifier
    max_velocity = 15  # 15max velocity of an ego by mutation
    min_velocity = 2  # min velocity of an ego by mutation
    max_init_velocity = 8  #8 max init velocity for new egos; also max velocity modifier for velocity mutations
    ...
    emergency_slow_down_amount = 0.3  # how much a car is slowing down for each step it spends in emergency mode
