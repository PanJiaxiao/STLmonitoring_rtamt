...
class MultiObjectiveProblem(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        #function4
        out["F"] = [-positive_count_1, -positive_count_2,positive_magnitude]
...
def get_pareto_front_and_best(res):
    ...
    # 找到最佳折衷解（基于两个目标的乘积）
    #function4
    scores = Object_1 * Object_2 * Object_3
    best_index = np.argmax(scores)
    best_individual = X[best_index]

    #function4
    return X, F, best_individual, best_index, Object_1, Object_2 ,Object_3
...
def save_results_to_excel(res, best_individual, filename="pymoo_optimization_results.xlsx"):
    ...
    X, F, _, best_index, Object_1, Object_2,Object_3 = get_pareto_front_and_best(res)

    # 创建数据框存储帕累托前沿解
    pareto_data = []
    for i, (ind, ob_1, ob_2,ob_3) in enumerate(zip(X, Object_1, Object_2,Object_3)):
        ...
        try:
            ...
            stats_data = {
                'Parameter': ['init_velocity_1', 'start_1', 'init_velocity_2', 'start_2','acc_1','acc_2',
                              'Object_1', 'Object_2','Object_3', 'Positive_Point_Ratio(%)'],
                'Value': [best_individual[0], best_individual[1], best_individual[2],
                          best_individual[3], best_individual[4], best_individual[5],
                          Object_1[best_index], Object_2[best_index],Object_3[best_index], positive_ratio]
            }
...
def analyze_results(res, save_excel=True, excel_filename="pymoo_optimization_results.xlsx"):
    ...
    # 获取帕累托前沿和最佳解
    X, F, best_individual, best_index, Object_1, Object_2 , Object_3 = get_pareto_front_and_best(res)

    print(f"\n最佳个体参数: {best_individual}")
    print(f"目标值 - 正数时间点数:a: {Object_1[best_index]},b:{Object_2[best_index]}, 平均幅度: {Object_3[best_index]:.3f}")
    ...
    #
    # plt.scatter(positive_counts, magnitudes, alpha=0.7, label='帕累托解')
    # plt.scatter(positive_counts[best_index], magnitudes[best_index],
    #             color='red', s=100, label='最佳解')
    # plt.xlabel('正数时间点数量')
    # plt.ylabel('平均正数幅度')
    # plt.title('帕累托前沿')
    # plt.legend()
