...
def get_init_scenario(scenario_type,vinit, start, end,acc,filename):
    ...
    # for stl monitor,need to remove all pp to generate randomly
...
def simulate(scenario: Scenario, problem_set: PlanningProblemSet, scenario_type,run_time,max_steps=200):
    ...
    while some_generator_running and step < max_steps:
        ...
        # TODO add monitor
        # last_solution_map.get(gen_object.planning_problem.planning_problem_id)[2]
        #print(last_solution_map.get(999)[0].position)
        #monitor(last_solution_map.get(999)[0].position[0],last_solution_map.get(1000)[0].position[0],"rob_save.csv")
        id=[]
        for i, problem in enumerate(planning_problems):
    ...
    return SimulationResult(images=images, scenario=scenario, problem_set=problem_set,
                            ...
                            algorithm="max_danger", emergency_stops=emergency_stops)
...
if __name__ == "__main__":
    ...
    result_name = f"{filename.split('.')[0]}_{args.criticality_mode}_{args.scenario_type}_{args.run_time}"
    ...
    #TODO old version
    #utils.save_for_stl(results, lanelet_network,filename, result_name)
