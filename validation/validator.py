...
#TODO: this funciton can not used in multi-branch road situation!!!
def get_successor_lanelets_notmerge(lanelet: Lanelet, ll_network: LaneletNetwork) -> List[Lanelet]:
    # TODO only need to check for remaining distance of the drivable area, do this with additional dist param
    full_lanelets = [lanelet]
    processed_ids = [lanelet.lanelet_id]
    full_split_lanelets=[]
        ...
        for ll in full_lanelets:
            if len(ll.successor) == 0:
                ...
                for succ_id in ll.successor:
                    ...
                    new_full_lls.append(full_lanelet)
                    processed_ids.append(succ_ll.lanelet_id)
                    if succ_ll not in full_split_lanelets:
                        full_split_lanelets.append(succ_ll)

        full_lanelets = new_full_lls
    return full_split_lanelets

#TODO: this funciton can not used in multi-branch road situation!!!
def get_predecessor_lanelets_notmerge(lanelet: Lanelet, ll_network: LaneletNetwork) -> List[Lanelet]:
    full_lanelets = [lanelet]
    processed_ids = [lanelet.lanelet_id]
    full_split_lanelets=[]
    while _have_lanelets_predecessors(full_lanelets, processed_ids):
        new_full_lls = []
        for ll in full_lanelets:
            if len(ll.predecessor) == 0:
                new_full_lls.append(ll)
            else:
                for pred_id in ll.predecessor:
                    pred_ll = ll_network.find_lanelet_by_id(pred_id)
                    full_lanelet = Lanelet.merge_lanelets(pred_ll,ll)               
                    new_full_lls.append(full_lanelet)
                    processed_ids.append(pred_ll.lanelet_id)
                    if pred_ll not in full_split_lanelets:
                        full_split_lanelets.insert(0,pred_ll)

        full_lanelets = new_full_lls
    return full_split_lanelets

def get_straight_lanelets(lanelet:Lanelet,full_lanelets:List[Lanelet],ll_network:LaneletNetwork)->List[Lanelet]:
    straight_lanets=[]
    straight_lanets.append(lanelet)
    # left_boundary = lanelet.left_vertices
    # right_boundary = lanelet.right_vertices 
    # e_left=left_boundary[-1]
    # e_right=right_boundary[-1]
    last_lanete=lanelet
    #print(len(full_lanelets),"full*****")
    for i in range(len(full_lanelets)): 
        left_boundary = last_lanete.left_vertices
        right_boundary = last_lanete.right_vertices 
        e_left=left_boundary[-1]
        e_right=right_boundary[-1]
        #print(e_left,"e_left*****")
        #print(e_right,"e_right*****")
        for j in range(len(full_lanelets)):
            left_boundary=full_lanelets[j].left_vertices  
            right_boundary=full_lanelets[j].right_vertices 
            s_left=left_boundary[0]
            s_right=right_boundary[0]
            ##对二元组的比较！！！！
            if operator.eq(s_left,e_left).all and operator.eq(s_right,e_right).all:
                #print("arrive")
                straight_lanets.append(full_lanelets[j])
                #前道路
                last_lanete=full_lanelets[j]
                if j==len(full_lanelets)-1:
                    #print(straight_lanets,"str_lan_return***********")
                    return straight_lanets
                full_lanelets.pop(j)
                break
            else:
                if j==len(full_lanelets)-1:
...
def _have_lanelets_predecessors(lanelets: List[Lanelet], visited_ll_ids: List[int]):
    have_predecessors = any(len(ll.predecessor) != 0 and any(
        ll_id not in visited_ll_ids for ll_id in ll.predecessor) for ll in lanelets)
    return have_predecessors
