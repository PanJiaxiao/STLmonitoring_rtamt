...
#TODO rewrite this function to include all pp(instead of only critical_key)
def write_scenario(filename, state_map, scenario: Scenario, problem_set,  sim_name):
    critical_pp = PlanningProblemSet()
    vehicle_constraints = VehModelParameters()
    length = vehicle_constraints.veh_length
    width = vehicle_constraints.veh_width
    ...
    os.makedirs(path, exist_ok=True)
    fw = CommonRoadFileWriter(scenario, critical_pp, 'MultiPlanner')
    output_filename = filename.split('.')[0] + f"-multi-planner-simulate.xml"
    fw.write_to_file(path + '/' + output_filename, OverwriteExistingFile.ALWAYS)
...
#
#TODO rewrite function for read data from xml in one time
def save_for_stl( filename: str, scenario_type,
                 run_time,xml_path,laneletnetwork:LaneletNetwork):
    # print("Arrive stl!!!!")
    if scenario_type == "change_lane":
        stl_dict = {
            # basic movenment
            ...
            "stl_17": {
                "phi_str": "((va_acc >= 0.001) and (abs(yr_a) < 0.001) and (vb_acc < 0.001) and (abs(yr_b) < 0.001)) or ((vb_acc >= 0.001) and (abs(yr_b) < 0.001) and (va_acc < 0.001) and (abs(yr_a) < 0.001))",
                "signal_str": "va_acc,yr_a,vb_acc,yr_b"},
            "stl_18": {"phi_str": "(va_acc < 0.001) and (abs(yr_a) < 0.001) and (vb_acc < 0.01) and (abs(yr_b) < 0.01)",
                       "signal_str": "va_acc,yr_a,vb_acc,yr_b"},
            ...
            # a车不加速，b车加速
            "stl_23": {
                "phi_str": "(abs(va*vb)>0.0) and (lan_o_b<0.01) and (distance<15) and (va_acc<0.001) and (lan_o_a<0.01)",
                "signal_str": "lan_o_b,distance,va_acc,lan_o_a,va,vb"},
            "stl_24": {
                "phi_str": "(abs(va*vb)>0.0) and (lan_o_a<0.01) and (distance<15) and (vb_acc>=0.001) and (lan_o_b<0.01)",
                "signal_str": "lan_o_a,distance,vb_acc,lan_o_b,va,vb"},
            # ab车不加速
            "stl_25": {
                "phi_str": "(abs(va*vb)>0.0) and (lan_o_b<0.01) and (distance<15) and (va_acc<0.001) and (lan_o_a<0.01)",
    ...
    elif scenario_type == "crossroad" and config.ScenarioGeneratorConfig.stl_type=="multi":
        # not add forward and backward constrane
        stl_dict = {
            # basic movenment
    ...
    elif scenario_type == "crossroad" and config.ScenarioGeneratorConfig.stl_type=="single":
        stl_dict = {
            "stl_0": {

                "phi_str": "(abs(va*vb)>0.0) and (yr_b*yr_a<0) and (abs(yr_b) >= 0.001) and (distance<20) and (abs(orientation_a-orientation_b)<1.5708) and "
                "(vb_acc >= 0.001) and (va_acc >= 0.001) and (abs(yr_a) >= 0.001)",
                "signal_str": "va,vb,yr_b,distance,yr_a,va_acc,vb_acc,orientation_b,orientation_a"},
            #双车转弯
            #一车不加速、一车加速
            "stl_1": {
                "phi_str": "(abs(va*vb)>0.0) and (yr_b*yr_a<0) and (abs(yr_b) >= 0.001) and (distance<20) and (abs(orientation_a-orientation_b)<1.5708) and (va_acc < 0.001) and (abs(yr_a) >= 0.001)and (vb_acc >= 0.001)",
                "signal_str": "va,vb,yr_b,distance,yr_a,va_acc,orientation_b,orientation_a,vb_acc"},
            #双车转弯
            #两车不加速
            "stl_2": {
                "phi_str": "(abs(va*vb)>0.0) and (yr_b*yr_a<0) and (abs(yr_b) >= 0.001) and (distance<20) and (abs(orientation_a-orientation_b)<1.5708) and (vb_acc < 0.001) and"
                "(va_acc < 0.001) and (abs(yr_a) >= 0.001)",
                "signal_str": "va,vb,yr_b,distance,yr_a,va_acc,vb_acc,orientation_b,orientation_a"},
            #转弯、加速
            #直行、加速
            "stl_3": {
                "phi_str": "(abs(va*vb)>0.0) and (abs(yr_b) < 0.001) and (distance<20) and (abs(orientation_a-orientation_b)<1.5708) and (vb_acc >= 0.001) and"
                " (va_acc >= 0.001) and (abs(yr_a) >= 0.001)",
                "signal_str": "va,vb,yr_b,distance,yr_a,va_acc,vb_acc,orientation_b,orientation_a"},
            #转弯、加速
            #直行、不加速
            ...
            "stl_5": {
                "phi_str": "(abs(va*vb)>0.0) and (abs(yr_b) < 0.001) and (distance<20) and (abs(orientation_a-orientation_b)<1.5708) and (vb_acc >= 0.001) and"
                "(va_acc < 0.001) and (abs(yr_a) >= 0.001)",
                "signal_str": "va,vb,yr_b,yr_a,distance,vb_acc,va_acc,orientation_b,orientation_a"},
            #转弯、不加速
            #直行、不加速
            "stl_6": {
                "phi_str": "(abs(va*vb)>0.0) and (abs(yr_b) < 0.001) and (distance<20) and (abs(orientation_a-orientation_b)<1.5708) and (va_acc < 0.001) and (vb_acc < 0.001) and (abs(yr_a) >= 0.001)",
                "signal_str": "va,vb,yr_b,distance,yr_a,va_acc,vb_acc,orientation_b,orientation_a"},
            #交叉道
            # 双车转弯
            # 两车加速
            "stl_7": {
                "phi_str": "(abs(va*vb)>0.0) and (yr_a*yr_b>0) and (abs(yr_b) >= 0.001)and (distance<20) and (abs(orientation_a-orientation_b)>=1.5708) and (vb_acc >= 0.001) and"
                "(va_acc >= 0.001) and (abs(yr_a) >= 0.001)",
                "signal_str": "va,vb,yr_b,distance,yr_a,va_acc,vb_acc,orientation_b,orientation_a"},
            # 双车转弯
            # a车不加速，b加速
            ...
            #直行、加速
            "stl_10": {
                "phi_str": "(abs(va*vb)>0.0) and (abs(yr_b) < 0.001) and (distance<20) and (abs(orientation_a-orientation_b)>=1.5708) and (abs(orientation_a-orientation_b)<=3.1416) and (vb_acc >= 0.001) and"
                "(va_acc >= 0.001) and (abs(yr_a) >= 0.001)",
                "signal_str": "va,vb,yr_b,distance,yr_a,va_acc,vb_acc,orientation_b,orientation_a"},
        ...
        stl_dict = {
            "stl_1": {"phi_str": "(va_acc>0.001)",
                      "signal_str": "va_acc"},
            "stl_2": {"phi_str": "(va_acc<-0.001)",
                      "signal_str": "va_acc"},
            "stl_3": {"phi_str": "((va_acc<0.001) and (va_acc>-0.001))",
                      "signal_str": "va_acc"},
            "stl_4": {"phi_str": "vb_acc>0.001",
                      "signal_str": "vb_acc"},
            "stl_5": {"phi_str": "vb_acc<-0.001",
                      "signal_str": "vb_acc"},
            "stl_6": {"phi_str": "(vb_acc<0.001 and vb_acc>-0.001)",
                      "signal_str": "vb_acc"},
            "stl_7": {"phi_str": "(va_acc>0.001) and (vb_acc>0.001)",
                      "signal_str": "va_acc,vb_acc"},
            "stl_8": {"phi_str": "(va_acc>0.001) and (vb_acc<-0.001)",
                      "signal_str": "va_acc,vb_acc"},
            "stl_9": {"phi_str": "(va_acc>0.001) and ((vb_acc<0.001 and vb_acc>-0.001))",
                      "signal_str": "va_acc,vb_acc"},
            "stl_10": {"phi_str": "(va_acc<-0.001) and (vb_acc<-0.001)",
                      "signal_str": "va_acc,vb_acc"},
            "stl_11": {"phi_str": "(va_acc>0.001) and (vb_acc<-0.001)",
                      "signal_str": "va_acc,vb_acc"},
            "stl_12": {"phi_str": "((va_acc<0.001 and va_acc>-0.001)) and ((vb_acc<0.001 and vb_acc>-0.001))",
                      "signal_str": "va_acc,vb_acc"},
            "stl_13": {
                "phi_str": "(abs(va*vb)>0.0) and ((distance<15) and (a_bef_b>0) and (va_acc>0.001)) or ((distance<15) and (a_bef_b<=0) and (vb_acc>0.001))",
                "signal_str": "a_bef_b,distance,va_acc,vb_acc,va,vb"},
            "stl_14": {
                "phi_str": "(abs(va*vb)>0.0) and ((distance<15) and (a_bef_b>0) and (vb_acc>0.001) ) or ((distance<15) and (a_bef_b<=0) and (va_acc>0.001) )",
                "signal_str": "a_bef_b,distance,va_acc,vb_acc,va,vb"},
            "stl_15": {
                "phi_str": "(abs(va*vb)>0.0) and ((distance<15) and (a_bef_b>0) and (va_acc>0.001) ) or ((distance<15) and (a_bef_b<=0) and (vb_acc>0.001) )",
                "signal_str": "a_bef_b,distance,va_acc,vb_acc,va,vb"},
            ...
            "stl_17": {
                ...
                "signal_str": "a_bef_b,distance,va_acc,vb_acc,va,vb"},
            "stl_18": {
                "phi_str": "(abs(va*vb)>0.0) and ((distance<15) and (a_bef_b>0) and ((vb_acc<0.001 and vb_acc>-0.001)))  or ((distance<15) and (a_bef_b<=0) and ((va_acc<0.001 and va_acc>-0.001))) ",
                "signal_str": "a_bef_b,distance,vb_acc,va_acc,va,vb"},
            "stl_19": {
                "phi_str": "(abs(va*vb)>0.0) and ((distance<15) and (a_bef_b>0) and (va_acc<-0.001)) or ((distance<15) and (a_bef_b<=0) and (vb_acc<-0.001))",
                "signal_str": "a_bef_b,distance,va_acc,vb_acc,va,vb"},
            ...
            "stl_24": {
                ...
                "signal_str": "a_bef_b,distance,vb_acc,va_acc,va,vb"},
            "stl_25": {
                "phi_str": "(abs(va*vb)>0.0) and ((distance<15) and (a_bef_b>0) and ((va_acc<0.001 and va_acc>-0.001))) or ((distance<15) and (a_bef_b<=0) and ((vb_acc<0.001 and vb_acc>-0.001))) ",
                "signal_str": "a_bef_b,distance,va_acc,vb_acc,va,vb"},
            "stl_26": {
                "phi_str": "(abs(va*vb)>0.0) and ((distance<15) and (a_bef_b>0) and (vb_acc>0.001)) or ((distance<15) and (a_bef_b<=0) and (va_acc>0.001)) ",
                "signal_str": "a_bef_b,distance,vb_acc,va_acc,va,vb"},
        ...
        stl_dict = {
            ...
            # 直行、不加速
            "stl_25": {
                "phi_str": "(abs(va*vb)>0.0) and (abs(yr_b) < 0.001) and (distance<15) and (abs(orientation_a-orientation_b)<1.5708) and (va_acc < 0.001) and (abs(yr_a) >= 0.001)",
                "signal_str": "va,vb,yr_b,distance,yr_a,va_acc,orientation_b,orientation_a"},
            "stl_26": {
                "phi_str": "(abs(va*vb)>0.0) and (abs(yr_a) >= 0.001) and (distance<15) and (abs(orientation_a-orientation_b)<1.5708) and (vb_acc < 0.001) and (abs(yr_b) < 0.001)",
                "signal_str": "va,vb,yr_a,distance,yr_b,vb_acc,orientation_b,orientation_a"},
            # 交叉道
            # 两车直行
            # 两车加速
    ...
    # path = result_path + sim_name
    # output_filename = filename.split('.')[0] + "-multi-planner-simulated.xml"
    # xml_file_path=path + '/' + output_filename
    crfr = CommonRoadFileReader(xml_path)
    scenario, planning_problem_set=crfr.open()
    # print("success open !!!!")
    dynamic_obstacles = scenario.dynamic_obstacles

    # print("dynamic_obstcles********",dynamic_obstacles[0].signal_series)
    # position;orientation;velocity;yaw_rate;acceleration;time_step
    time_step=[]
    min_time_step=5000
    for i in dynamic_obstacles:
        time_step.append(i.prediction.final_time_step)
    if time_step[0]<time_step[1]:
        min_time_step=time_step[0]
    else:
        min_time_step=time_step[1]
    #print("time_step", time_step)
    #print("max_time_step",min_time_step)
    lenth = len(stl_dict)
    robs = [[] for _ in range(lenth)]
    for t in range(1,min_time_step+1):
        data_1=dynamic_obstacles[0].state_at_time(t)
        data_2=dynamic_obstacles[1].state_at_time(t)
        #print("arrive t")
        if data_1.velocity<0.001 or data_2.velocity<0.001:
            break
        for index, stl_index in enumerate(stl_dict, start=1):
            #print("arrive index")
            phi_str = stl_dict[stl_index]["phi_str"]
            ...
            for item in signal_str_list:
                if item == "position_a":
                    ...
                    data_result.append(data_1.velocity)
                elif item == "orientation_a":
                    data_result.append(data_1.orientation)
                elif item == "position_b":
                    position = data_2.position
                    data_result.append(position)
                elif item == "position_x_b":
                    position = data_2.position
                    data_result.append(position.position[0])
                    ...
                    data_result.append(data_2.velocity)
                elif item == "orientation_b":
                    data_result.append(data_2.orientation)
                elif item == "distance":
                    position_a = data_1.position
                    position_b = data_2.position
                    a_x = position_a[0]
                    a_y = position_a[1]
                    b_x = position_b[0]
                    b_y = position_b[1]
                    ...
                    data_result.append(data_1.acceleration)
                elif item == "vb_acc":
                    data_result.append(data_2.acceleration)
                elif item == "a_bef_b":
                    position_a = data_1.position
                    position_b = data_2.position
                    data_temp = compute_a_bef_b(laneletnetwork, position_a, position_b, scenario_type)
                    ...
                    position_a = data_1.position
                    orientation_a = data_1.orientation
                    data_temp = compute_lan_orientaion(laneletnetwork, position_a, orientation_a)
                    data_result.append(data_temp)
                elif item == "lan_o_b":
                    position_a = data_2.position
                    orientation_a = data_2.orientation
                    data_temp = compute_lan_orientaion(laneletnetwork, position_a, orientation_a)
                    data_result.append(data_temp)
                elif item == "yr_a":
                    data_result.append(data_1.yaw_rate)
                elif item == "yr_b":
                    data_result.append(data_2.yaw_rate)
    #
    #     # TODO monitor complete
            rob=monitor(data_result, filename, str(index), phi_str, signal_str_list, scenario_type, run_time)
            #print("robs&&&&&", rob)
            robs[index-1].append(str(rob))


    for index, stl_index in enumerate(stl_dict):
        filepath = "./rob_result/" + scenario_type + "/" + config.ScenarioGeneratorConfig.file_vinit + "/rob_save"  + str(
            index) + "-run_time" + str(run_time) + ".csv"
        # filepath_2 = "./rob_result/" + scenario_type + "/" + config.ScenarioGeneratorConfig.file_vinit + "/raw/" + filename + str(
        #     index) + "-run_time" + str(run_time) + ".csv"
        os.makedirs("./rob_result/" + scenario_type + "/" + config.ScenarioGeneratorConfig.file_vinit ,
                    exist_ok=True)

        # 创建DataFrame，将list作为一列
        df = pd.DataFrame(robs[index], columns=["rob"])

        # 保存为CSV，不保存索引
        df.to_csv(filepath, index=False, encoding='utf-8')

        # if os.path.exists(filepath):
        #     data_write = [robs[index]]
        #     with open(filepath, mode='a', encoding='utf-8', newline='') as file:
        #         writer = csv.writer(file)
